...
data Error = EmptyString | UnknownReq
...
mkTopic
  :: Text
  -> Either Error Topic
mkTopic t = if (t == "") then Left EmptyString else Right (Topic t)
...
mkCommentText t = if (t == "") then Left EmptyString else Right (CommentText t)
