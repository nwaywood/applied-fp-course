...
confPortToWai
  :: Conf
  -> Int
confPortToWai conf = fromIntegral (getPort $ port conf)
...
instance Semigroup PartialConf where
  _a <> _b = PartialConf
    -- { pcPort       = case (pcPort _a, pcPort _b) of
    --     (_, Just b) -> Just b
    --     (v, Nothing) -> v
    -- , pcDBFilePath = case (pcDBFilePath _a, pcDBFilePath _b) of
    --     (_, Just b) -> Just b
    --     (v, Nothing) -> v
    -- }
...
partialConfDecoder = D.withCursor $ \c -> do
    o <- D.down c
    p <- D.fromKey "pcPort" D.integral o
    fp <- D.fromKey "pcDBFilePath" D.string o
    pure PartialConf {
        pcPort = Just (Last $ Port p),
        pcDBFilePath = Just (Last $ DBFilePath fp)
    }
